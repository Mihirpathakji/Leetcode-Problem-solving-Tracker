...
public:
    ...
    int maxDepth(TreeNode* root) {
        ...
        int left_Subtree_Height = maxDepth(root->left);//1   0 
        
        //Calculate the height of the right subtree from root node.

        int right_Subtree_Height = maxDepth(root->right);// 0   2

        //Total Height after including the root Node.

        return max(left_Subtree_Height,right_Subtree_Height)+1;//1  1   2 
